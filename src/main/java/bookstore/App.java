...
public class App {
    ...
    /**
     * Cleanly tears down the (expensive) database resources. Called from
     * Main's finally block so connections close even if run() throws.
     */
    ...
    private void saveToDb(Editable item) {
        ...
        } else if (item instanceof NintendoGame) {
            entity = ((NintendoGame) item).toEntity();
        } else if (item instanceof NintendoHardware) {
            entity = ((NintendoHardware) item).toEntity();
        }
    ...
    public void listAny() {
        ...
        while (choice != 99) {
            ...
            switch(choice) {
                ...
                case 6: filterClass = NintendoGameEntity.class; break;
                case 7: filterClass = NintendoHardwareEntity.class; break;
                default: System.out.println("Invalid selection."); continue;
            }
    ...
    private void printEntityAsDto(ProductEntity entity) {
        ...
        } else if (entity instanceof NintendoGameEntity) {
            System.out.println(NintendoGame.fromEntity((NintendoGameEntity) entity));
        } else if (entity instanceof NintendoHardwareEntity) {
            System.out.println(NintendoHardware.fromEntity((NintendoHardwareEntity) entity));
        }
    }
        ...
        try {
            ...
            if (idx >= 0 && idx < entities.size()) {
                ...
                if (entity instanceof BookEntity) {
                    ...
                    NintendoGame dto = NintendoGame.fromEntity((NintendoGameEntity) entity);
                    dto.edit(this.input);
                    em.merge(dto.toEntity());
                } else if (entity instanceof NintendoHardwareEntity) {
                    NintendoHardware dto = NintendoHardware.fromEntity((NintendoHardwareEntity) entity);
                    dto.edit(this.input);
                    em.merge(dto.toEntity());
                }
    ...
    public void populate() {
        ...
        try {
            for (int i = 0; i < 2; i++) {
                ...
                // Nintendo Game (Niche Department)
                NintendoGameEntity game = new NintendoGameEntity(
                        faker.book().title(),                    // title
                        faker.number().randomDouble(2, 30, 70),  // price
                ...
                NintendoHardwareEntity hardware = new NintendoHardwareEntity(
                        faker.commerce().productName(),          // name
                        faker.number().randomDouble(2, 50, 350), // price
                        faker.number().numberBetween(3, 15),     // copies
                        "Switch",                                // platform
            ...
            em.getTransaction().commit();
            System.out.println("Seeding complete. All products, including the Nintendo niche, persisted directly to MySQL disk volume.");
        } catch (Exception e) {
            if (em.getTransaction().isActive()) {
                em.getTransaction().rollback();
            }
    ...
    public SaleableItem findItem(SaleableItem item) {
        ...
        for (ProductEntity entity : entities) {
            ...
            if (entity instanceof BookEntity) {
                ...
                pojo = NintendoGame.fromEntity((NintendoGameEntity) entity);
            } else if (entity instanceof NintendoHardwareEntity) {
                pojo = NintendoHardware.fromEntity((NintendoHardwareEntity) entity);
            }
